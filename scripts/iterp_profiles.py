...
if include_impurity:
   ni_frac,nimp_frac = calc_nfracs(zeff=zeff,zimp=zimp)
   idens = ni_frac*dens 
   impdens = nimp_frac*dens 
else:
   idens = dens 
...
if include_impurity:
    output_iterdb(rhot[:],rhop[:],dens[:],te[:],idens[:],ti[:],base_name,'9999','0.0',vrot=omega_tor,nimp = impdens)
else:
    output_iterdb(rhot[:],rhop[:],dens[:],te[:],dens[:],ti[:],base_name,'9999','0.0',vrot=omega_tor)
