...
class fieldfile():
    #class constructor
    def __init__(self,file,pars,fast):
        self.pars=pars
        self.file=file
        self.set_gridcounts()
        self.set_sizes()
    ...
    def redirect(self,file):
        ...
        self.f=open(file,'rb')
        self.tfld=[]
        self.tlen=int(getsize(self.file)/(self.leapfld+self.tesize))
        self.get_timearray()
        self.reset_tinds()
    ...
    def get_minmaxtime(self):
        try: 
            ...
            min=float(self.te.unpack(self.f.read(self.tesize))[1])
            self.f.seek((int(getsize(self.file))/(self.leapfld+self.tesize)-1)*(self.leapfld+self.tesize),0)
            max=float(self.te.unpack(self.f.read(self.tesize))[1])
            return min,max
    ...
    def get_time(self,tind):
        self.f.seek(tind*(self.leapfld+self.tesize),0)
        time=self.te.unpack(self.f.read(self.tesize))[1]
        return time

    #set current timestep
    def set_time(self,time,tind):
        self.time=time
        self.tind=tind #self.get_tind()
